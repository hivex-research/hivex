...
def construct_command(config_path, experiment_name="", force=True, train=True):
    ...
    run_id = "--run-id=" + (f"{experiment_name}/{midfix}/{config_path.stem}" if experiment_name != "" else config_path.stem)
...
def create_config_files(experiment_name: str, config_path: Path, output_folder_path: Path, run_count: int = 1):

    tmp_file_paths = []
    with open(ROOT.as_posix() + config_path.as_posix(), "r") as f:
        configuration = yaml.safe_load(f)
        
        configuration["env_settings"]["env_path"] = ROOT.as_posix() + configuration["env_settings"]["env_path"]
        ...
        for test_run_id in range(run_count):
            for _, parameter_permutation in enumerate(parameter_permutations):
                ...
                for key, value in zip(all_range_parameter_keys, parameter_permutation):
                    ...
                    name_suffix += "_" + str(key[-1]) + "_" + str(value).replace(".", "")
...
def create_training_config_files(experiment_name: str, train_config_path: Path, train_run_count: int):

    output_folder_path = ROOT / Path("configs/mlagents/tmp/train")
    output_folder_path.mkdir(parents=True, exist_ok=True)

    return create_config_files(experiment_name=experiment_name, config_path=train_config_path, output_folder_path=output_folder_path, run_count=train_run_count)

def create_test_config_files(test_config_path: Path, experiment_name: str, test_run_count: int):
    ...
    temp_test_config_file_paths = create_config_files(experiment_name=experiment_name, config_path=test_config_path, output_folder_path=output_folder_path, run_count=test_run_count)
    
    for test_config_path in temp_test_config_file_paths:
        test_params_split = test_config_path.stem.replace(f"{experiment_name}_", "").replace("_test", "").split("_")        
        test_params = [test_params_split[index - 1] + "_" + test_params_split[index] for index in range(len(test_params_split)) if test_params_split[index].isnumeric()]
        # remove id
        ...
        for train_folder_name in os.listdir(train_results_dir):
            ...
            #     continue
            train_params_split = train_folder_name.replace(f"{experiment_name}_", "").replace("_train", "").split("_")
            train_params = [train_params_split[index - 1] + "_" + train_params_split[index] for index in range(len(train_params_split)) if train_params_split[index].isnumeric()]
            # remove id
            ...
            if match_count == len(train_params):                
                with open(test_config_path, "r") as f:
                    configuration = yaml.safe_load(f)
                    configuration["behaviors"]["Agent"]["init_path"] = str((train_results_dir / train_folder_name / "Agent/checkpoint.pt").absolute())
                f.close()
...
def load_hivex_config(config_file_path: Path):
    ...
    return {"experiment_name": config['experiment_name'],
